...
class HeaderBarWindow(Gtk.Window):
    ...
    def msg_show(self,event,bt,box,attbtrmv,msg_unicode,web,grid):    	
    	self.add(self.grid)  
    	if "www.youtube.com" not in msg_unicode:     		
    		self.resize(500, 350)
    		box.add(self.toggle_stay)
    	else:
    		web.set_size_request(500,400)
    		self.resize(700, 600)
    		grid.attach(web, 0, 2, 1, 1)

    	self.show_all()
    	attbtrmv.hide();
    	bt.hide()

    def get_source(self,web,webobj, frame):    
    	print "loading"	
    ...
    def create_textview(self,box2,hb):
		...
		self.msg = Gtk.Button()
		self.msg.connect("clicked", self.msg_show,self.msg,box,self.attachedbtrmv,msg_unicode,web,self.grid)	
		...
		if config_note.getColorRevertTitle():
			self.photo.set_from_file("/usr/share/gnome-shell/extensions/turbonote@iksws.com.br/icons/ic_action_camera" + config_note.getColorOver() + ".png")		
		else:
		...
		if "www.youtube.com" in msg_unicode:					
			#https://www.youtube.com/watch?v=maNb1JGyxjQ						
			you1 = msg_unicode.split("?v=")
			you2 = you1[1].split("\n")			
			web.open("http://www.youtube.com/embed/" + you2[0])
			web.connect("load-finished",self.get_source,web)			
			
			if (new_msg != "N"):							
				web.set_size_request(500,400)
				self.resize(700, 500)			
				self.grid.attach(web, 0, 2, 1, 1)
		else:
